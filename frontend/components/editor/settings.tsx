...
import { Model, ModelProvider } from 'docs-hub-shared-models';
...
interface NextModel extends Model {
	label: string;
	value: string;
}
...
export function SettingsProvider({ children }: { children: ReactNode }) {
	...
	const [models, setModels] = useState<NextModel[]>([]);
	const [model, setModel] = useState<NextModel>(models[0]);
		...
		request('/api/model/id', {
			...
			.then((res) => {
				const list = (Array.isArray(res.data) ? res.data : []).map(
					(e: ModelProvider) => ({ ...e, label: e.name, value: e.id }),
				);
				setModel(list[0]);
				setModels(list);
			});
	...
	return (
		<SettingsContext.Provider value={{ keys, model, models, setKey, setModel }}>
			{children}
...
export function SettingsDialog() {
	...
	const { keys, model, models, setKey, setModel } = useSettings();
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent>
				<DialogHeader>
					<DialogTitle className="text-xl">{tDocument('settings')}</DialogTitle>
					<DialogDescription>
						{tDocument('configModel')}
					</DialogDescription>
				</DialogHeader>
				...
				<form className="space-y-10" onSubmit={handleSubmit}>
					...
					<div className="space-y-4">
						...
						<div className="space-y-4">
							{renderApiKeyInput('openai', tDocument('apiKey'))}
							...
							<div className="group relative">
								...
								<Popover open={openModel} onOpenChange={setOpenModel}>
									...
									<PopoverContent className="w-full p-0">
										<Command>
											<CommandInput placeholder={tDocument('searchModel')} />
											<CommandEmpty>No model found.</CommandEmpty>
											<CommandList>
												<CommandGroup aria-multiselectable>
													{models.map((m: NextModel) => (
														<CommandItem
															key={m.value}
															value={m.value}
															onSelect={() => {
